...
## Компоненты
- **Frontend** — Next.js 15 (app router) и React 18. Общается с FastAPI через `NEXT_PUBLIC_API_BASE_URL` по REST/JSON. Встроенные Next API routes обращаются к внешнему HiAnime (aniwatch) для поиска, расписаний, эпизодов и м3u8‑источников, а также маппинга импортируемых списков AniList.
- **Backend** — FastAPI с async SQLAlchemy 2 и Alembic. Использует PostgreSQL, отдает статические аватары по `/media/avatars`, выполняет миграции и проверку БД на старте. RBAC применяется через зависимость `require_enforced_permission`.
...
3. Источники воспроизведения и поиск по HiAnime идут через Next API routes и не проходят через бэкенд. Для проигрывателя требуется `NEXT_PUBLIC_PROXY_URL` (endpoint `/m3u8-proxy` вне репозитория) для корректного HLS‑проксирования.

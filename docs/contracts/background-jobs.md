...
| retry | До `max_attempts` (по умолчанию 3) с линейным backoff `backoff_seconds * attempt` |
...
- Попытки: до 3, с задержками 1s/2s/3s (ограничено `backoff_seconds * max_attempts`).

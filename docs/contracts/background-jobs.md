...
| retry | До `max_attempts` (по умолчанию 3) с задержкой `min(backoff_seconds * attempt, backoff_seconds * max_attempts)`, где `attempt` — номер повторной попытки начиная с 1 |
...
- Попытки: до 3; после каждой неудачи счётчик `attempts` увеличивается, задержка = `min(backoff_seconds * attempts, backoff_seconds * max_attempts)` (при `backoff_seconds=1` это 1s → 2s → 3s). Backoff линейный, а не экспоненциальный.

...
| retry | До `max_attempts` (по умолчанию 3) с задержкой `min(backoff_seconds * attempt, backoff_seconds * max_attempts)` |
...
- Попытки: до 3; задержка перед попыткой `n` вычисляется как `min(backoff_seconds * n, backoff_seconds * max_attempts)` (по умолчанию 1s → 2s → 3s).

...
## Proxy Layer — FAIL
- `/api/search/suggestion` does not translate upstream 4xx/5xx or network failures to proxied statuses (unhandled `httpx` errors bubble as 500 instead of 4xx/502) (backend/app/api/proxy/search.py:18-62).
- `/api/anime/{anime_id}` and `/api/anime/{anime_id}/episodes` bypass 4xx/5xx proxying: upstream 4xx/5xx (or network errors) return 200/500 instead of forwarded status/502 because responses are not wrapped in HTTPExceptions (backend/app/api/proxy/anime.py:10-66).
...
### Verdict
**P2.4 BLOCKED** — Proxy layer still fails to proxy upstream errors per contract (see issues above).
